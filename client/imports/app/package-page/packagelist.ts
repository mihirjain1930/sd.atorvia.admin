...
export class ListPackageComponent extends MeteorComponent implements OnInit, OnDestroy {
    ...
    private setOptionsSub() {
        this.optionsSub = Observable.combineLatest(
            ...
            this.call("packages.find", options, {}, searchString, (err, res) => {
    ...
    activate(item: Package) {
        ...
        Meteor.call("packages.activate", item._id, (err, res) => {
            if (err) {
    ...
    deactivate(item: Package) {
        ...
        Meteor.call("packages.deactivate", item._id, (err, res) => {
            if (err) {
    ...
    deletePackage(item: Package) {
        ...
        Meteor.call("packages.delete", item._id, (err, res) => {
            ...
            //so trigger change detection manually to update dom
            this.changeDetectorRef.detectChanges();
            showAlert("Package has been deleted.", "success");
        })
    }
