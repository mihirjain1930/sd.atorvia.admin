...
export class ListPractitionerComponent extends MeteorComponent implements OnInit {
    items: User[];
    ...
    private setOptionsSub() {
        this.optionsSub = Observable.combineLatest(
            ...
            this.localStorageService.set("practitioner-list.options", {
                pageSize: pageSize,
                curPage: curPage,
                nameOrder: nameOrder,
    ...
    activate(user: User) {
        if (! confirm("Are you sure to activate this practitioner?")) {
            return false;
        }
    ...
    deactivate(user: User) {
        if (! confirm("Are you sure to deactivate this practitioner?")) {
            return false;
        }
